...
class SimpleGraph {
    ...
    public ArrayList<Vertex> WeakVertices() {
        ArrayList<Vertex> listForSingles = new ArrayList<>();
        for (int i = 0; i < vertex.length; i++) {
            Vertex v = checkVertex(i);
            if (v != null) {
                listForSingles.add(v);
            }
        }
        return listForSingles;
    }

    private Vertex checkVertex(int v) {
        ArrayList<Integer> listNeighbours = listVertexD(v);
        for (int i = 0; i < listNeighbours.size(); i++) {
            for (int j = 0; j < listNeighbours.size(); j++) {
                if (i != j && m_adjacency[listNeighbours.get(i)][listNeighbours.get(j)] == 1) {
                    return null;
                }
            }
        }
