...
class SimpleTree<T> {
    ...
    public ArrayList<T> EvenTrees() {
        ...
        SimpleTreeNode<T> startNode = this.Root;
        ArrayList<T> resultTrees = new ArrayList<>();

        return searchChild(startNode, resultTrees);
    }

    private ArrayList<T> searchChild(SimpleTreeNode<T> startNode, ArrayList<T> resultTrees ) {
        if (startNode.Children == null) {
            return new ArrayList<>();
        }
        for (int i = 0; i < startNode.Children.size(); i++) {
            List<SimpleTreeNode<T>> listNodes = new ArrayList<>();
            routeTree(startNode.Children.get(i), listNodes);
            if ((listNodes.size()+1) % 2 == 0) {
                resultTrees.add(startNode.Children.get(i).Parent.NodeValue);
                resultTrees.add(startNode.Children.get(i).NodeValue);
            }
            searchChild(startNode.Children.get(i), resultTrees );
        }
